Yes. The main problem is that your current `page.tsx` uses `course` everywhere, but `course` is **never defined inside `SingleCoursePage()`**.

You already correctly use `getCourseBySlug()` in `generateMetadata()`. You need to do the same in the actual page component and call `notFound()` if the slug doesn't exist.

There are also a couple of things I would clean up:

* `course` must come from the URL slug.
* `notFound()` is currently imported but unused.
* `Testimonials` should ideally receive course-specific testimonials if your data supports them.
* `course.lead.image` should **not** be used yet, as requested.
* The hero image can temporarily remain commented out if you don't have reliable course images.
* Related-course content is currently hard-coded, so it will be identical for every course.
* The page should be `async` because you're using the current Next.js `params: Promise<...>` pattern.

## Replace your entire `app/courses/[slug]/page.tsx` with this
...
Your original component had:

```tsx
export default function SingleCoursePage() {
```

but then immediately tried to use:
...
There was no `course` variable.

It now correctly has:
...
```text
/courses/compelling-communication
```
...
```tsx
getCourseBySlug("compelling-communication")
```

returns that course's data.

Likewise:

```text
/courses/generative-ai-in-business
```

gets the Generative AI course.

---

## One more thing: your `data/courses.ts` is now very important

The page above assumes your course data has this general structure:
...
export const courses = [
  {
    ...
    lead: {
      ...
      role: "...",
      bio: "...",
      // image: "/courses/leads/person.jpg", // add later
    },
  },

  {
    ...
    sections: [
      // Generative AI-specific content
    ],
...
**This is what actually makes each course different.**

The `[slug]/page.tsx` should mostly be a **template**. Your `data/courses.ts` should contain the actual course-specific information.

### Don't do this

```tsx
if (slug === "compelling-communication") {
  // huge amount of JSX
}

if (slug === "generative-ai-in-business") {
  // another huge amount of JSX
}
```

That becomes very difficult to maintain.
...
### About the images I commented out

I deliberately left this commented:
...
So you **do not need `course.lead.image` in your data yet**.
...
lead: {
  ...
  role: "...",
  bio: "...",
  image: "/courses/leads/example.jpg",
}
```

you can remove the placeholder:

```tsx
<div className="mb-4 h-32 w-32 ... bg-gray-200" />
```

and uncomment the `<img>`.

---

## One likely remaining source of errors

If you still get TypeScript errors after replacing `page.tsx`, they will most likely be coming from **`data/courses.ts`**, not this page.

For example, if your current `Course` type doesn't define:
...
**Send me your current `data/courses.ts` file next.** I can then restructure that file properly for your actual courses (`Compelling Communication`, `Generative AI in Business`, `Business Essentials`, etc.) so every `/courses/[slug]` page gets completely different content without creating a mess of conditional JSX.
