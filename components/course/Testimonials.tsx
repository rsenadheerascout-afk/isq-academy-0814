export default function Testimonials() {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 font-montserrat">
      {testimonials.map((t) => (
        <div key={t.id} className="border border-gray-200 p-8 flex flex-col items-center text-center bg-white shadow-sm hover:shadow-md transition-shadow">
          <span className="text-5xl text-gray-300 font-serif leading-none mb-2">“</span>
          <div className="w-16 h-16 bg-gray-200 rounded-full mb-4"></div>
          <h4 className="font-bold text-gray-900">{t.name}</h4>
          <p className="text-xs text-gray-500 mb-6">{t.role}</p>
